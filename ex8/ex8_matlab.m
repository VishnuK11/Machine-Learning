...
% it does not, right-click the 'ex8' folder and select 'Open' before proceding 
...
% and then find values that have very low probability and hence can be considered 
...
% 
% 
%     Once you have completed the code in |estimateGaussian.m|, the code 
% below will visualize the contours of the fitted Gaussian distribution. You should 
% get a plot similar to Figure 2. From your plot, you can see that most of the 
% examples are in the region with the highest probability, while the anomalous 
% examples are in the regions with lower probabilities.
...
% 
%     You should now complete the code in |selectThreshold.m|. For this, 
% we will use a cross validation set $\{(x^{(1)}_{cv},y^{(1)}_{cv}),\ldots, (x^{(m)}_{cv},y^{(m)}_{cv})\}$, 
...
% |yval|.
% 
%     The function |selectThreshold.m| should return two values; the first 
% is the selected threshold $\epsilon$. If an example $x$ has a low probability 
% $p(x) < \epsilon$, then it is considered to be an anomaly. The function should 
% also return the $F_1$ score, which tells you how well you're doing on finding 
% the ground truth anomalies given a certain threshold. For many different values 
...
% will try many different values of $\epsilon$ and select the "best" based $\epsilon$ 
...
% 
% 
% *Implementation Note*: In order to compute $tp,\; fp,\text{and }fn$, you 
% may be able to use a vectorized implementation rather than loop over all the 
% examples. This can be implemented by MATLAB's equality test between a vector 
...
% are 0 by using: |sum(v == 0)|. You can also apply a logical |and| operator to 
% such binary vectors. For instance, let |cvPredictions| be a binary vector of 
% the size of your cross validation set, where the $i$-th element is 1 if your 
% algorithm considers $x^{(i)}_{cv}$ an anomaly, and 0 otherwise. You can then, 
% for example, compute the number of false positives using:| fp = sum((cvPredictions 
% == 1) & (yval == 0))|.
% 
%     Once you have completed the code in |selectThreshold.m|, the code below 
% will detect and circle the anomalies in the plot (Figure 3).
...
% 
% *<https://grouplens.org/datasets/movielens/ MovieLens 100k Dataset> from 
% GroupLens Research.
...
% 
%     To help you understand the matrix |Y|, the code below will compute 
% the average movie rating for the first movie (Toy Story) and output the average 
% rating to the screen. Throughout this part of the exercise, you will also be 
% working with the matrices, |X| and |Theta|:
% 
% |X = |$\left\lbrack \begin{array}{c}-{\text{ }\left(x^{\left(1\right)} 
% \right)}^T -\\-{\text{ }\left(x^{\left(2\right)} \right)}^T -\\\vdots \\-{\text{ 
% }\left(x^{\left(n_m \right)} \right)}^T -\end{array}\right\rbrack$,    |Theta 
% = |$\left\lbrack \begin{array}{c}-{\;\left(\theta^{\left(1\right)} \right)}^T 
% -\\-{\;\left(\theta^{\left(2\right)} \right)}^T -\\\vdots \\-{\;\left(\theta^{\left(n_u 
...
xlabel('Users');
%% 
% The $i$-th row of |X| corresponds to the feature vector $x^{(i)}$ for 
% the $i$-th movie, and the $j$-th row of |Theta| corresponds to one parameter 
% vector $\theta^{(j)}$, for the $j$-th user. Both $x^{(i)}$ and $\theta^{(j)}$ 
% are $n$-dimensional vectors. For the purposes of this exercise, you will use 
% $n = 100$, and therefore, $x^{(i)} \in\mathbb{R}^{100}$ and $\theta^{(j)} \in\mathbb{R}^{100}$. 
% Correspondingly, |X| is a $n_m\times 100$ matrix and |Theta| is a $n_u\times 
% 100$ matrix.
...
% 
%     You will complete the code in |cofiCostFunc.m| to compute the cost 
% function and gradient for collaborative filtering. Note that the parameters 
% to the function (i.e., the values that you are trying to learn) are |X| and 
% |Theta|. In order to use an off-the-shelf minimizer such as |fmincg|, the cost 
% function has been set up to unroll the parameters into a single vector |params|. 
% You had previously used the same vector unrolling method in the neural networks 
...
% by unrolling them into a single vector. After you have completed the code to 
% compute the gradients, the code below will run a gradient check (|checkCostFunction|) 
...
% 
%     To perform the vectorization, you might find this helpful: You should 
% come up with a way to compute all the derivatives associated with $x^{(i)}_1,x^{(i)}_2,\ldots,x^{(i)}_n$, 
...
% movie as:
% 
% $${\left({\mathrm{X}}_{\mathrm{grad}} \left(\mathrm{i},:\right)\right)}^T 
...
% 
%     Concretely, you can set |idx = find(R(i,:)==1)| to be a list of all 
% the users that have rated movie $i$. This will allow you to create the temporary 
% matrices |Theta_temp = Theta(idx,:)| and |Y_temp = Y(i,idx)| that index into 
...
% 
% ${\mathrm{X}}_{\textrm{grad}} \left(i,:\right)=\left(\mathrm{X}\left(i,:\right)*{\textrm{Theta}}_{\textrm{temp}}^T 
% -{\mathrm{Y}}_{\textrm{temp}} \right)*{\textrm{Theta}}_{\textrm{temp}}$.
% 
% (Note: The vectorized computation above returns a row-vector instead.) 
% After you have vectorized the computations of the derivatives with respect to 
% $x^{(i)}$, you should use a similar method to vectorize the derivatives with 
% respect to $\theta^{(j)}$ as well.
...
% by
% 
% $$J\left(x^{(i)},\ldots,x^{(n_m)},\;\theta^{(1)},\ldots,\theta^{(n_u)}\right)=\frac{1}{2}\sum_{(i,j):r(i,j)=1}\left((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\right)^2+\left(\frac{\lambda}{2}\sum_{j=1}^{n_u}{\sum_{k=1}^{n}{(\theta_k^{(j)})^2}}\right)+\left(\frac{\lambda}{2}\sum_{i=1}^{n_m}{\sum_{k=1}^{n}{(x_k^{(i)})^2}}\right)$$
% 
% You should now add regularization to your original computations of the 
% cost function, $J$. After you are done, the code below will run your regularized 
